...
**可见性开关（玩家操作，非枪包内容）**：配件加工台的染色页签提供「可见」按钮，可把当前模块标记为隐藏。隐藏标志存物品 NBT（`Hidden`），装上枪后只跳过该模块**自身**的渲染——**子挂载不受连坐**（隐藏护木后其上的导轨配件照常显示，由作者自行设计挂点避免浮空）。属性、命中、枪口锚点不受影响——纯外观开关。两个特例：

- **机匣**：机匣物品在加工台上架成枪时同样携带染色、皮肤与隐藏标志；隐藏机匣只藏枪身本体，已装模块照常渲染。**裸枪（只有机匣、无任何模块、弹匣空）单独放进合成格可拆回机匣**——制造品质、染色、皮肤、隐藏标志全部保留（见 §3.9）。
- **挂件**：隐藏且**带皮肤**时，链条保留（物理摆动照常），只隐藏 `pendant` 骨骼内的内容，皮肤渲染在 pendant 骨骼的动画位置（皮肤顶替吊坠、跟着链条晃动，适用于吊坠被皮肤替代的挂饰搭配）。隐藏且无皮肤 → 挂件整体不渲染。
...
### 3.9 时装工坊皮肤（Armourer's Workshop 兼容）

本 mod 可选兼容 **时装工坊（Armourer's Workshop，AW）**：玩家把任意模块物品放进 AW 的皮肤台（skinning table）应用一个 `armourers:item` 类型的皮肤后，安装到枪上时该模块就不再渲染 GeckoLib 模型，而是渲染皮肤。AW 未安装时一切照旧——皮肤只是一段无人解析的 NBT。

- **玩家操作**：AW 皮肤台 + 模块物品 + AW 皮肤物品。AW 自带配方 `SkinningItemRecipe` 对任意非皮肤物品生效，CPT 不需要任何配合代码。
- **皮肤优先于染色/发光**：被皮肤的模块跳过 CPT 的染色通道与发光通道（皮肤自带贴图与涂装方案）。挂点位置、枪口锚点、属性计算完全不受影响。
- **皮肤跟随模块拆装往返**：物品是唯一权威，枪的 NBT 只保存整个 `ArmourersWorkshop` 描述符的**逐字副本**（不解析内部内容，AW 的涂装/染色数据完整保留）。
- **自定义渲染器优先于皮肤**：声明了 `ModuleRenderOverrides` 的模块仍走自定义渲染。
...
**main 骨跟随**：模块模型最外层如果有 `main` 骨，皮肤会画在 `main` 骨的变换空间里——动画驱动 `main`（后坐、歪斜等）时皮肤跟着模型一起动，不会脱节。静止时 `main` 是恒等变换（无旋转/缩放时轴的平移项自消），皮肤对齐与现在完全一致。模型没有最外层 `main` 骨时皮肤照旧画在挂点定位符原点。

**机匣皮肤**：机匣同样可以皮肤化——机匣物品在 AW 皮肤台上应用皮肤后**再上架成枪**（皮肤在 stageGun 时随染色/隐藏标志一起拷进枪 NBT）。带皮肤的机匣不再渲染枪身本体，而是把皮肤画在枪的持枪位姿上（动画 pass 照常运行，模块挂点跟随后坐；已装模块照常渲染）。无皮肤、或皮肤暂不可用（AW 缺失/异步烘焙窗口）时回落枪身模型。**隐藏机匣**＝只藏本体：枪身消失、模块照常（皮肤也不画——藏的就是整个机匣外观）。

**裸枪拆解（合成表）**：只有机匣的枪（无任何已装模块、弹匣空）单独放进任意合成格即拆回机匣物品——制造品质、染色、AW 皮肤、隐藏标志**逐字节保留**（与工作台拆模块的物化拷贝同一条链路）。这是已上架机匣唯一的回收路径：带任何模块或弹药的枪不能拆，先清空再拆。

**挂件皮肤与隐藏的互动**：挂件隐藏且带皮肤时走「皮肤顶替吊坠」特例——链条照常物理摆动，`pendant` 骨骼的内容隐藏，皮肤渲染在 pendant 骨骼的动画位置。给挂件做皮肤的作者应把皮肤对准吊坠（皮肤画在 pendant 骨骼原点，即最末链节下方）。
